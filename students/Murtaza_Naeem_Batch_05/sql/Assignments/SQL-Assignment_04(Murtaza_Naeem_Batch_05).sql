...
SELECT p.category_id, p.product_name, p.list_price, ROUND(c.avg_price, 2) AS category_avg
...
AvgOrders AS (
    SELECT AVG(order_count * 1.0) AS avg_count FROM StaffOrders
)
...
SELECT store_id, order_year AS year, ROUND(total_revenue, 2) AS total_revenue
FROM StoreYearRevenue
...
CREATE TABLE sales.loyalty_cards (
    card_number   INT UNIQUE NOT NULL,  -- unique card number
    customer_id   INT NOT NULL,
    points        INT NOT NULL CHECK (points >= 0),  -- no negative points
    tier          VARCHAR(10) NOT NULL CHECK (tier IN ('Bronze', 'Silver', 'Gold')), -- valid tiers only
